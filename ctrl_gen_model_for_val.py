...
class CtrlGenModelVal(object):
    """Control
    """

    def __init__(self, inputs, vocab, gamma, lambda_g, want_code, hparams=None):
    ...
    def _build_model(self, inputs, vocab, gamma, lambda_g):
        ...
        c_ = label_connector(self._want_code - labels)
        h = tf.concat([c, z], 1)
        h_ = tf.concat([c_, z], 1)
        ...
        loss_g_clas = tf.nn.sigmoid_cross_entropy_with_logits(
            labels=tf.to_float(self._want_code-labels), logits=soft_logits)
        loss_g_clas = tf.reduce_mean(loss_g_clas)

        # Accuracy on soft samples, for training progress monitoring
        accu_g = tx.evals.accuracy(labels=self._want_code-labels, preds=soft_preds)
        ...
        accu_g_gdy = tx.evals.accuracy(
            labels=self._want_code-labels, preds=gdy_preds)
