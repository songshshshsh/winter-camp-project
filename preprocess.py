...
def preprocess_data():
	...
	val_posts = np.array([process_posts(data, words, stop) for data in raw_data['posts'][(int)(tot_data_size * train_ratio): (int)(tot_data_size * (train_ratio + val_ratio))]])
	test_label = np.array([hash_label(data) for data in raw_data['type'][(int)(tot_data_size * (train_ratio + val_ratio)):]])
	test_posts = np.array([process_posts(data, words, stop) for data in raw_data['posts'][(int)(tot_data_size * (train_ratio + val_ratio)):]])
