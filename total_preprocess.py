...
def process_posts(posts, words):
        ...
        for post in posts.split('|||'):
                ...
                for punc_word in punc_set:
                        post = post.replace(punc_word, ' {}'.format(punc_word))
                current_post = []
...
def preprocess_data(filepath):
	...
	save(filepath + '/process/all_data', label, posts)
	f = open(filepath + '/process/words', 'w')
	for word in words:
		f.write(word + '\n')
	f.close()
